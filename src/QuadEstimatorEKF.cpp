...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    Quaternion<float> q_t = Quaternion<float>::FromEulerYPR(ekfState(6), pitchEst, rollEst);
    Quaternion<float> d_q_bar = q_t.IntegrateBodyRate(gyro, dtIMU);

    
    float predictedPitch = d_q_bar.Pitch();
    float predictedRoll = d_q_bar.Roll();
    ekfState(6) = d_q_bar.Yaw(); // yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    //std::cout << "QUAD_EKF_NUM_STATES =  " << QUAD_EKF_NUM_STATES << endl;
     //std::cout << "curState.x =  " << curState(0) << endl;
    accel = attitude.Rotate_BtoI(accel);
    predictedState(0) = curState(0) + (curState(3) * dt);
    predictedState(1) = curState(1) + (curState(4) * dt);
    predictedState(2) = curState(2) + (curState(5) * dt);
    predictedState(3) = curState(3) + accel.x * dt;
    predictedState(4) = curState(4) + accel.y * dt;
    //predictedState(5) = curState(5) + (  accel.z ) * dt;
    predictedState(5) = curState(5) + dt * accel.z - dt * CONST_GRAVITY;
    //std::cout << "predictedState.x =  " << predictedState(0) << endl;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  gPrime(0,3)=dt; gPrime(1,4)=dt; gPrime(3,5)= dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
   ...
   // hPrime(0,0)=1;hPrime(1,1)=1;hPrime(2,2)=1;hPrime(3,3)=1;hPrime(3,3)=1;hPrime(3,3)=1;hPrime(3,3)=1;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
